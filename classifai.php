...
function classifai_define( $name, $value ) {
	if ( ! defined( $name ) ) {//  - Constant name, as a string, should be used along with `defined()`.
		define( $name, $value );//  - Constant name, as a string, should be used along with `defined()`.
	}
}
...
function classifai_autoload() {
	if ( classifai_can_autoload() ) {
		require_once( classifai_autoloader() );//  - File inclusion using custom function ( `classifai_autoloader()` ). Must return local file source, as external URLs are prohibited on WordPress VIP. Probably needs manual inspection.
...
function classifai_can_autoload() {
	if ( file_exists( classifai_autoloader() ) ) {
		...
		error_log(//  - error_log() found. Debug code should not normally be used in production.
...
function classifai_autorun() {
	if ( classifai_autoload() ) {
		...
		if ( defined( 'WP_CLI' ) && WP_CLI ) {
			require_once CLASSIFAI_PLUGIN_DIR . '/includes/Classifai/Command/ClassifaiCommand.php'; //  - File inclusion using custom constant (`CLASSIFAI_PLUGIN_DIR`). Probably needs manual inspection.
		}
	} else {
		if ( defined( 'WP_CLI' ) && WP_CLI ) {
			try {
...
function classifai_autoload_notice() {
	...
	error_log( get_error_install_message() );//  - error_log() found. Debug code should not normally be used in production.
}
