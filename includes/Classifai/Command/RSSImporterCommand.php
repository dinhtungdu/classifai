...
class RSSImporterCommand extends \WP_CLI_Command {
	...
	public function get_url_meta( $url ) {
		...
		$options['timeout']              = 60; //  - Detected high remote request timeout. `timeout` is set to `60`.

		$request_url = 'https://mercury.postlight.com/parser?url=' . urlencode( $url ); //  - urlencode() should only be used when dealing with legacy applications rawurlencode() should now be used instead. See http://php.net/manual/en/function.rawurlencode.php and http://www.faqs.org/rfcs/rfc3986.html
		$response    = wp_remote_get( $request_url, $options ); //  - wp_remote_get() is highly discouraged, please use vip_safe_wp_remote_get() instead.
